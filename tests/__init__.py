...
def clear_stream(stream: TextIO):
    """Clear the contents of a given stream if it is seekable."""
    if stream.seekable():
        stream.seek(0)
        stream.truncate(0)
...
def reset_store(store: FileStorage, file_path="file.json"):
    """Reset the items in the given store and reloads it."""
...
def read_text_file(file_name):
    """Read the contents of a given file and returns it as a string."""
    lines = []
...
def write_text_file(file_name, text):
    """Write a text to a given file name."""
